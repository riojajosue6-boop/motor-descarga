...
<body>
    ...
    <div class="container">
        ...
        <div id="privacy-sec" class="legal-content">
            ...
            <ul>
                <li><strong>Cookies:</strong> Utilizamos cookies para personalizar anuncios y analizar nuestro tráfico (Google AdSense).</li>
                <li><strong>Datos de Usuario:</strong> No almacenamos los videos que descargas ni conservamos registros personales.</li>
            </ul>
        </div>
        <div id="terms-sec" class="legal-content">
            ...
            <ul>
                <li><strong>Uso Responsable:</strong> Esta herramienta es para uso personal y educativo. El usuario es responsable por los derechos de autor.</li>
                <li><strong>Límites:</strong> Los videos no deben exceder los 10 minutos para garantizar la estabilidad del servidor.</li>
            </ul>
        </div>
    </div>
...
def api_extract():
    ...
    # --- TU EJÉRCITO DE PROXIES RESIDENCIALES ---
    proxy_users = [
    ...
    for user in proxy_users:
        ...
        try:
            ...
            # Si el error es por duración, no seguimos intentando otros proxies
            if "muy largo" in str(e):
                return jsonify({"success": False, "error": "El video excede los 10 minutos."})
            # De lo contrario, intentamos con el siguiente proxy
            continue

    return jsonify({"success": False, "error": "YouTube está bloqueando la conexión. Intenta de nuevo."})
