...
# --- TU DISEÑO PREMIUM ORIGINAL (TEXTOS LEGALES INTACTOS) ---
HTML_PREMIUM = """
<!DOCTYPE html>
<html lang="es">
<head>
    ...
    <style>
        ...
        .options { display: flex; gap: 10px; margin: 25px 0; }
...
<body>
    ...
    <div class="container">
        ...
        <div id="privacy-sec" class="legal-content">
            ...
            <ul>
                ...
                <li><strong>Google AdSense:</strong> Como proveedor externo, Google utiliza cookies para publicar anuncios...</li>
                <li><strong>Datos de Usuario:</strong> No almacenamos los videos que descargas ni conservamos registros...</li>
            </ul>
        </div>
        ...
        <div id="terms-sec" class="legal-content">
            ...
            <ul>
                <li><strong>Uso Responsable:</strong> Esta herramienta está diseñada para descargar contenido de uso personal...</li>
                <li><strong>Sin Garantías:</strong> No garantizamos que el servicio sea ininterrumpido...</li>
                <li><strong>Limitación de Responsabilidad:</strong> No nos hacemos responsables por el uso indebido...</li>
            </ul>
        </div>
    </div>
    ...
    <script>
        ...
        async function processVideo() {
            ...
            b.disabled = true; s.innerText = "⏳ Tu motor propio está analizando el enlace...";
            ...
            try {
                ...
                if(data.success) {
                    ...
                    s.innerText = "❌ Error: El video es muy largo o la plataforma lo bloqueó.";
                }
...
def api_extract():
    ...
    # REGLAS DE TU PROPIA API (yt-dlp)
    ydl_opts = {
        'quiet': True,
        'no_warnings': True,
        'format': 'best' if fmt == 'mp4' else 'bestaudio/best',
        # Límite de seguridad: No procesar videos de más de 10 minutos (600 seg)
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            # Buscamos el link directo en los formatos
            link = info.get('url')
