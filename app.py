...
<body>
    ...
    <script>
        ...
        async function processVideo() {
            ...
            b.disabled = true; p.style.display = 'none'; err.style.display = 'none'; box.style.display = 'none'; dBtn.disabled = true;
        ...
        async function generateDownload(url, tipo) {
            const s = document.getElementById('status');
            const err = document.getElementById('errorMessage');
            s.innerText = "🚀 Generando descarga..."; err.style.display = 'none';
            try {
                ...
                if(data.url) {
                    ...
                    } else { window.open(data.url, '_blank'); s.innerText = "✅ Descarga abierta"; }
                } else { s.innerText = ""; err.style.display = 'block'; }
...
@app.route('/ads.txt')
def ads_txt():
    # Código directo de respaldo para evitar errores de archivo
